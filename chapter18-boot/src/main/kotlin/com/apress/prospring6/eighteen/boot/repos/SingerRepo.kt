...
SOFTWARE.
*/
package com.apress.prospring6.eighteen.boot.repos
 ...
 */
interface SingerRepo : JpaRepository<Singer, Long> {
    ...
    fun findByFirstNameAndLastName(@Param("fn") firstName: String, @Param("ln") lastName: String): Iterable<Singer>
    ...
    fun findByLastName(@Param("ln") lastName: String): Iterable<Singer>

    @Query("select s from Singer s where s.lastName like %?1%")
    fun findByLastNameLike(lastName: String): Iterable<Singer>

    @Query("select s from Singer s where s.birthDate=:date")
    fun findByBirthDate(@Param("date") date: LocalDate): Iterable<Singer>
}
