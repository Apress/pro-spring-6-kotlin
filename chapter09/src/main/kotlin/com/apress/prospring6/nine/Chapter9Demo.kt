 ...
 */
object Chapter9Demo {
    private val LOGGER = LoggerFactory.getLogger(Chapter9Demo::class.java)
    @JvmStatic
    fun main(args: Array<String>) {
        AnnotationConfigApplicationContext(TransactionCfg::class.java).use { ctx ->
            val service =
                ctx.getBean(
                    AllService::class.java
                )
            LOGGER.info(" ---- Listing singers:")
            service.findAllWithAlbums()
                .forEach{ s: Singer ->
                    LOGGER.info(
                        s.toString()
                    )
