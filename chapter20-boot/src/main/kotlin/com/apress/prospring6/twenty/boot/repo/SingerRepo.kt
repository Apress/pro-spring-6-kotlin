 ...
 */
interface SingerRepo : ReactiveCrudRepository<Singer, Long> {
    @Query("select * from singer where first_name=:fn and last_name=:ln")
    fun findByFirstNameAndLastName(@Param("fn") firstName: String, @Param("ln") lastName: String): Mono<Singer>

    @Query("select * from singer where first_name=:fn")
    fun findByFirstName(@Param("fn") firstName: String): Flux<Singer>

    @Query("select * from singer where last_name=:ln")
    fun findByLastName(@Param("ln") lastName: String): Flux<Singer>

    @Query("select * from singer where birth_date=:ln")
    fun findByBirthDate(@Param("bd") birthDate: LocalDate): Flux<Singer>
}
