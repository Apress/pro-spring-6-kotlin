 ...
 */
@Disabled
class SingerHandlerTest {
    private val handlerClient = WebTestClient
        .bindToServer()
        .baseUrl("http://localhost:8081/handler/singer")
        .build()

    @Test
    fun shouldReturnAllSingers() {
        handlerClient.get()
            ...
            .consumeWith { body: EntityExchangeResult<ByteArray?>? ->
                LOGGER.info(
                    "Result: {} ",
                    body
                )
    ...
    @Test
    fun shouldReturnJohn() {
        handlerClient.get()
            .uri { uriBuilder: UriBuilder ->
                uriBuilder.queryParam(
                    "fn",
                    "John"
                ).queryParam("ln", "Mayer").build()
            }
            .accept(MediaType.APPLICATION_JSON)
            .exchange()
            .expectStatus().isOk()
