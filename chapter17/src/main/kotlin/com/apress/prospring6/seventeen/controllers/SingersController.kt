...
import com.apress.prospring6.seventeen.controllers.OneSingerController.Companion.setPhoto
...
import com.apress.prospring6.seventeen.util.UrlUtil.encodeUrlPathSegment
...
open class SingersController(private val singerService: SingerService, private val messageSource: MessageSource) {
    @GetMapping //@RequestMapping(method = RequestMethod.GET)
    open fun list(uiModel: Model): String {
        val singers = singerService.findAll().sortedBy { it.id }
    ...
    open fun showCreateForm(uiModel: Model): String {
    ...
    open fun create(
        @Valid singerForm: SingerForm, bindingResult: BindingResult, uiModel: Model,
        ...
        return if (bindingResult.hasErrors()) {
            ...
            val s = Singer().apply {
                ...
                // Process file  upload
                if (singerForm.file?.isEmpty != true) {
                    setPhoto(this, singerForm.file!!)
                }
            }
            val created = singerService.save(s)
            "redirect:/singer/" + encodeUrlPathSegment(
                created.id.toString(),
                httpServletRequest
            )
        }
    ...
    open fun showSearchform(criteria: CriteriaDto): String {
        return "singers/search"
    }

    @GetMapping(value = ["/go"])
    open fun processSubmit(
        ...
        return if (result.hasErrors()) {
            ...
            val singers = singerService.getByCriteriaDto(criteria)
            if (singers.isEmpty()) {
                result.addError(
                    FieldError(
                        "criteriaDto",
