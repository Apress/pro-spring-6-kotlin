...
@NamedQueries(
	NamedQuery(
		name = Singer.FIND_SINGER_BY_ID, query = "select distinct s from Singer s " +
				"left join fetch s.albums a " +
				"left join fetch s.instruments i " +
				"where s.id = :id"
	), NamedQuery(
		name = Singer.FIND_ALL_WITH_ALBUM, query = "select distinct s from Singer s " +
				"left join fetch s.albums a " +
				"left join fetch s.instruments i"
	)
)
class Singer : AbstractEntity() {
	...
	private var albums: MutableSet<Album> = mutableSetOf()
	...
	private var instruments: MutableSet<Instrument> = mutableSetOf()
	...
	override fun equals(other: Any?): Boolean {
		...
		if (firstName != singer.firstName) return false
		return if (lastName != singer.lastName) false else birthDate == singer.birthDate
	}
