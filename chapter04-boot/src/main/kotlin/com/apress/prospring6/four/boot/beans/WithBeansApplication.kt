...
class StandardOutMessageRenderer : com.apress.prospring6.two.decoupled.MessageRenderer {
...
open class WithBeansApplication {
    companion object {
        private val logger: org.slf4j.Logger = org.slf4j.LoggerFactory.getLogger(WithBeansApplication::class.java)

        @JvmStatic
        fun main(args: Array<String>) {
            val ctx = SpringApplication.run(arrayOf(WithBeansApplication::class.java), args)
            val mr: com.apress.prospring6.two.decoupled.MessageRenderer =
                ctx.getBean("messageRenderer", com.apress.prospring6.two.decoupled.MessageRenderer::class.java)
            mr.render()
        }
    }
}